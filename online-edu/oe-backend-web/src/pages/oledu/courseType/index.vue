  ...
  <t-card>
    <t-form layout="inline" :data="queryForm" :label-width="50" colon @submit="onSubmit">
      <t-form-item label="编码" name="code">
        <t-input v-model="queryForm.code" placeholder="请输入编码" clearable />
      </t-form-item>
    ...
    <course-type-form ref="courseTypeFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
const courseTypeFormRef = ref(null);
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await courseTypeMgt.page(queryForm.value);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await courseTypeMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  courseTypeFormRef.value.show(id);
};

const handleBatchDelete = async () => {
  // const ids = selectedRowKeys.value.map((item) => item).join();
  await courseTypeMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
