<template>
  <t-card>
    <t-form layout="inline" :data="queryForm" :label-width="70" colon @submit="onSubmit">
      <t-form-item label="敏感词" name="sensitiveWord">
        <t-input v-model="queryForm.sensitiveWord" placeholder="" clearable />
      </t-form-item>
    ...
    <sensitive-word-form ref="sensitiveWordFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
const sensitiveWordFormRef = ref(null);
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await sensitiveWordMgt.page(queryForm.value);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await sensitiveWordMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  sensitiveWordFormRef.value.show(id);
};
...
const handleBatchDelete = async () => {
  ...
  await sensitiveWordMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
