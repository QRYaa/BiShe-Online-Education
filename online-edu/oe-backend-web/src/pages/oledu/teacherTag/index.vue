<template>
  <t-card>
    ...
    <teacher-tag-form ref="teacherTagFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
import TeacherTagForm from './components/TeacherTagForm.vue';
...
const teacherTagFormRef = ref(null);
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await teacherTagMgt.page(queryForm.value);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await teacherTagMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  teacherTagFormRef.value.show(id);
};
...
const handleBatchDelete = async () => {
  ...
  await teacherTagMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
