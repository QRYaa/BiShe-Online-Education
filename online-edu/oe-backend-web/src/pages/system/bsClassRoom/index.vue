<template>
  <t-card>
    ...
    >
      <template #name="{ row }">
        <t-avatar shape="round" :hide-on-load-failed="true" :image="imgPre + row.cover">{{ row.name }}</t-avatar>
        {{ row.name }}
      </template>
      <template #status="{ row }">
      ...
      <template #op="{ row }">
        <t-space>
          <t-link theme="primary" @click="showForm(row.id)">修改</t-link>
          <t-popconfirm
            theme="danger"
            content="确认删除该数据吗"
            :confirm-btn="{
    ...
    <bs-class-room-form ref="bsClassRoomFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
import BsClassRoomForm from './components/BsClassRoomForm.vue';
...
const bsClassRoomFormRef = ref(null);
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await bsClassRoomMgt.page(queryForm.value);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await bsClassRoomMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  bsClassRoomFormRef.value.show(id);
};
...
const handleBatchDelete = async () => {
  ...
  await bsClassRoomMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
