  ...
  >
    <template #body>
      ...
      <t-form ref="formRef" :data="formData" :rules="rules" :label-width="100" @submit="onSubmit">
        ...
        <t-form-item label="前缀" name="preCode">
          <t-input v-model="formData.preCode" :style="{ width: '480px' }" placeholder="" />
        </t-form-item>
...
const init = async (id: any) => {
  const data = await classPrototypeMgt.get(id);
  formData.value = data;
};
...
const onSubmit = async (ctx: SubmitContext) => {
  if (ctx.validateResult === true) {
    if (formData.value.id > 0) {
      await classPrototypeMgt.update(formData.value);
    } else {
      await classPrototypeMgt.save(formData.value);
    }
    emit('fetch-data');
    close();
...
const rules: Record<string, FormRule[]> = {
  ...
  preCode: [{ required: true, message: '请输入前缀', type: 'error' }],
