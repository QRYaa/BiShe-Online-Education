<template>
  <t-card>
    ...
    <classType-form ref="classTypeFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
import classTypeForm from './components/ClassTypeForm.vue';
...
const classTypeFormRef = ref(null);
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await classTypeMgt.page(queryForm.value);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await classTypeMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  classTypeFormRef.value.show(id);
};
...
const handleBatchDelete = async () => {
  ...
  await classTypeMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
