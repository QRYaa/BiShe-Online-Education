  ...
  >
    <template #body>
      ...
      <t-form ref="formRef" :data="formData" :rules="rules" :label-width="100" @submit="onSubmit">
        ...
        <t-form-item label="值" name="content">
          <t-input v-model="formData.content" :style="{ width: '480px' }" placeholder="请输入设置值" />
        </t-form-item>
...
const init = async (id: any) => {
  const data = await settingMgt.get(id);
  formData.value = data;
};
...
const onSubmit = async (ctx: SubmitContext) => {
  if (ctx.validateResult === true) {
    if (formData.value.id > 0) {
      await settingMgt.update(formData.value);
    } else {
      await settingMgt.save(formData.value);
    }
    emit('fetch-data');
    close();
