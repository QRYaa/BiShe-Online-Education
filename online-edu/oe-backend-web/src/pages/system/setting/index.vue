  ...
  <t-card>
    <t-form layout="inline" :data="state.queryForm" :label-width="50" colon @submit="onSubmit">
      ...
      <t-form-item label="编码" name="code">
        <t-input v-model="state.queryForm.code" placeholder="请输入编码" clearable />
      </t-form-item>
    ...
    <setting-form ref="settingFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
import * as settingMgt from '@/api/system/settingMgt';
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await settingMgt.page(state.queryForm);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await settingMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  settingFormRef.value.show(id);
};
...
const handleBatchDelete = async () => {
  ...
  await settingMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
