  ...
  <t-card>
    <t-form layout="inline" :data="queryForm" :label-width="50" colon @submit="onSubmit">
      <t-form-item label="名称" name="name">
        <t-input v-model="queryForm.name" placeholder="请输入标签名称" clearable />
      </t-form-item>
    ...
    <tag-form ref="tagFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await tagMgt.page(queryForm.value);
    data.value = content;
...
const handleBatchDelete = async () => {
  ...
  await tagMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
