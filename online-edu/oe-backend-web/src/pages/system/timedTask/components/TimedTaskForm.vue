  ...
  >
    <template #body>
      ...
      <t-form ref="formRef" :data="formData" :rules="rules" :label-width="100" @submit="onSubmit">
        ...
        <t-form-item label="处理器名称" name="processorName">
          <t-input v-model="formData.processorName" :style="{ width: '480px' }" placeholder="" />
        </t-form-item>
        <t-form-item label="cron表达式" name="cron">
          <t-input v-model="formData.cron" :style="{ width: '480px' }" placeholder="" />
        </t-form-item>
      </t-form>
    </template>
  </t-dialog>
...
const init = async (id: any) => {
  const data = await timedTaskMgt.get(id);
  formData.value = data;
};
...
const onSubmit = async (ctx: SubmitContext) => {
  if (ctx.validateResult === true) {
    if (formData.value.id > 0) {
      await timedTaskMgt.update(formData.value);
    } else {
      await timedTaskMgt.save(formData.value);
    }
    emit('fetch-data');
    close();
