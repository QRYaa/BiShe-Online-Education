<template>
  <t-card>
    ...
    <timed-task-form ref="timedTaskFormRef" @fetch-data="fetchData" />
  </t-card>
</template>
...
import * as timedTaskMgt from '@/api/system/timedTaskMgt';
import { useDictStore } from '@/store';

import TimedTaskForm from './components/TimedTaskForm.vue';
...
const timedTaskFormRef = ref(null);
...
const fetchData = async () => {
  ...
  try {
    const { content, totalItems } = await timedTaskMgt.page(queryForm.value);
    data.value = content;
...
const onConfirmDelete = async (id: any) => {
  // 真实业务请发起请求
  await timedTaskMgt.del(id);
  await fetchData();
  MessagePlugin.success('删除成功');
};

const showForm = (id?: any) => {
  timedTaskFormRef.value.show(id);
};
...
const handleBatchDelete = async () => {
  ...
  await timedTaskMgt.batchDel(selectedRowKeys.value);
  await fetchData();
  MessagePlugin.success('删除成功');
...
const resumeJob = async (row: any) => {
  await timedTaskMgt.resumeJob(row);
  await fetchData();
  MessagePlugin.success('开启成功');
};
const pauseJob = async (row: any) => {
  await timedTaskMgt.pauseJob(row);
  await fetchData();
