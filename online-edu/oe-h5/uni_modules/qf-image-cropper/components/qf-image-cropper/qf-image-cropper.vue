<template>
	<!--  -->
	<view v-if="showCropper" class="image-cropper" :style="{ zIndex }" @wheel="cropper.mousewheel">
		...
		<slot />
		<view class="fixed-bottom" :style="{ zIndex: initData.area.zIndex + 99 }">
			<view class="choose-btn" @click="showCropper=false" style="color: #333;">
			...
			<block v-else-if="!!imgSrc">
				<view class="choose-btn" @click="chooseImage">
					<view class="choose-btn-content">
						重选
					</view>
				</view>
				<view class="button-btn" @click="cropClick">确定</view>
			</block>
